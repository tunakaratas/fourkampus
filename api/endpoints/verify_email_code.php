...
try {
    ...
    // Kodu kontrol et
    $check_stmt = $db->prepare("SELECT id, expires_at, used, verified FROM email_verification_codes WHERE email = ? AND code = ? ORDER BY created_at DESC LIMIT 1");
    ...
    // Kodu doğrulandı olarak işaretle (used=1 yapma)
    try {
        $mark_verified_stmt = $db->prepare("UPDATE email_verification_codes SET verified = 1 WHERE id = ?");
        if ($mark_verified_stmt) {
            $mark_verified_stmt->bindValue(1, $row['id'], SQLITE3_INTEGER);
            @$mark_verified_stmt->execute();
        }
    } catch (Exception $e) {
        error_log("Failed to mark code as verified: " . $e->getMessage());
    }
