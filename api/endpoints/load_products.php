...
if (!isset($_SESSION['admin_id'])) {
    ...
    if (!$db_path || !file_exists($db_path)) {
        ...
        // Hala bulunamadıysa, tüm veritabanlarını tara ve en çok ürüne sahip olanı bul
        if (!$db_path || !file_exists($db_path)) {
                    ...
                    foreach ($folders as $folder) {
                        ...
                        if ($test_db && file_exists($test_db) && is_readable($test_db)) {
                            try {
                                ...
                                $count = (int)($test_conn->querySingle("SELECT COUNT(*) FROM products WHERE club_id = $club_id") ?: 0);
                                $test_conn->close();
    ...
    // Toplam sayı
    $total_count = (int)($db->querySingle("SELECT COUNT(*) FROM products WHERE club_id = $club_id") ?: 0);
    ...
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        ...
        if (!empty($row['image_path'])) {
            $row['image_url'] = 'index.php?action=product_media&file=' . urlencode($row['image_path']);
        }
        $products[] = $row;
    }
    ...
    error_log("load_products.php error: " . $e->getMessage() . " | Trace: " . $e->getTraceAsString());
    echo json_encode([
        'success' => false,
        'error' => $e->getMessage(),
        'file' => $e->getFile(),
    ...
    http_response_code(500);
    error_log("load_products.php fatal error: " . $e->getMessage() . " | Trace: " . $e->getTraceAsString());
    echo json_encode([
        'success' => false,
