...
function get_all_communities($useCache = true) {
    ...
    foreach ($dirs as $dir) {
        ...
        try {
            ...
            $qr_code_url = '/api/qr_code.php?type=community&id=' . urlencode($dir);
...
try {
    ...
    if ($requested_university_id !== '') {
        ...
        foreach ($community_folders as $folder_path) {
            ...
            if (in_array($community_id, $excluded_dirs)) {
                ...
                // Diğer detaylar - direkt dosya yolu (domain yok)
                $logo_path = null;
                if (!empty($settings['club_logo'])) {
                    $logo_path = '/communities/' . $community_id . '/' . ltrim($settings['club_logo'], '/');
                }
                
                $image_url = null;
                if (!empty($settings['club_image'])) {
                    $image_url = '/communities/' . $community_id . '/' . ltrim($settings['club_image'], '/');
                }
                ...
                $qr_code_url = '/api/qr_code.php?type=community&id=' . urlencode($community_id);
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        ...
        try {
            ...
            // Logo path - direkt dosya yolu (domain yok)
            $logo_path = null;
            if (!empty($settings['club_logo'])) {
                $logo_path = '/communities/' . $community_id . '/' . ltrim($settings['club_logo'], '/');
            }
            
            // Image URL - direkt dosya yolu (domain yok)
            $image_url = null;
            if (!empty($settings['club_image'])) {
                $image_url = '/communities/' . $community_id . '/' . ltrim($settings['club_image'], '/');
            }
    ...
    foreach ($paginated_communities as $community) {
        ...
        // QR kod API URL'i - direkt path
        $qr_code_url = '/api/qr_code.php?type=community&id=' . urlencode($community['id']);
