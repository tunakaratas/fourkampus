...
// Direkt dosya yolu döndür - domain adı kullanma
// build_absolute_url fonksiyonu kaldırıldı - artık direkt path kullanılıyor
function build_absolute_url($path) {
    // Domain kullanılmıyor, direkt path döndür
    if (empty($path)) {
        return null;
    }
    // Path zaten relative, normalize et ve döndür
    return '/' . ltrim($path, '/');
}
