...
if (!isset($_SESSION['admin_id'])) {
                    ...
                    foreach ($folders as $folder) {
                        ...
                        if ($test_db && file_exists($test_db) && is_readable($test_db)) {
                            try {
                                ...
                                $count = (int)($test_conn->querySingle("SELECT COUNT(*) FROM members WHERE club_id = $club_id") ?: 0);
                                $test_conn->close();
    ...
    // Toplam sayı
    $total_count = (int)($db->querySingle("SELECT COUNT(*) FROM members WHERE club_id = $club_id") ?: 0);
    
    // Üyeleri çek
    $sql = "SELECT id, club_id, full_name, email, student_id, phone_number, registration_date 
            FROM members 
    ...
    error_log("load_members.php error: " . $e->getMessage() . " | Trace: " . $e->getTraceAsString());
    echo json_encode([
        'success' => false,
        'error' => $e->getMessage(),
        'file' => $e->getFile(),
    ...
    http_response_code(500);
    error_log("load_members.php fatal error: " . $e->getMessage() . " | Trace: " . $e->getMessage());
    echo json_encode([
        'success' => false,
