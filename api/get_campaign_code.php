...
try {
    ...
    // Üyelik kontrolü
    if ($campaign['requires_membership'] == 1) {
        // Üyelik durumunu kontrol et (membership_statuses tablosundan)
        try {
            $membership_stmt = $db->prepare("SELECT status FROM membership_statuses WHERE user_id = ? AND club_id = 1");
            if (!$membership_stmt) {
                throw new Exception('SQL hazırlama hatası: ' . $db->lastErrorMsg());
            }
            $membership_stmt->bindValue(1, $user_id, SQLITE3_TEXT);
            $membership_result = $membership_stmt->execute();
            if (!$membership_result) {
                throw new Exception('SQL çalıştırma hatası: ' . $db->lastErrorMsg());
            }
            $membership = $membership_result->fetchArray(SQLITE3_ASSOC);
            
            if (!$membership || ($membership['status'] != 'member' && $membership['status'] != 'approved')) {
                $db->close();
