...
try {
    ...
    // Cache key oluştur
    $cache_key = "campaigns_list_{$club_id}_{$offset}_{$limit}";
    ...
    $count_cache_key = "campaigns_count_{$club_id}";
    $total_count = $cache->remember($count_cache_key, $cache_ttl, function() use ($db, $club_id) {
        return $db->querySingle("SELECT COUNT(*) FROM campaigns WHERE club_id = $club_id") ?: 0;
    });
    
    // Kampanyaları çek
    $query = $db->prepare("SELECT * FROM campaigns WHERE club_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?");
