<?php
/**
 * Sorunlu Veritabanı Dosyalarını Yeniden Oluştur
 * İzin sorunu olan veritabanı dosyalarını yeniden oluşturur
 */

$communities_dir = __DIR__ . '/../communities';
$problematic = ['aaa', 'aabb'];

echo "🔧 Sorunlu veritabanı dosyalarını yeniden oluşturuyorum...\n\n";
...
foreach ($problematic as $community_id) {
    ...
    try {
        ...
        if (!is_writable($dir)) {
            @chmod($dir, 0755);
            if (!is_writable($dir)) {
                throw new Exception("Klasör yazılabilir değil: $dir");
            }
        }
        ...
        // Dosyanın gerçekten oluşturulduğunu ve okunabilir olduğunu kontrol et
        if (!file_exists($db_path)) {
        ...
        if (!is_readable($db_path)) {
            @chmod($db_path, 0666);
            if (!is_readable($db_path)) {
                throw new Exception("Veritabanı dosyası okunabilir değil");
            }
        }
