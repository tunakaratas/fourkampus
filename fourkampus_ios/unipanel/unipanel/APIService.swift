...
private func loadFromCache<T: Decodable>(_ type: T.Type, for endpoint: String) -> T? {
    ...
    func getCampaigns(communityId: String? = nil, universityId: String? = nil) async throws -> [Campaign] {
        ...
        #endif
        let result: [Campaign] = try await request(endpoint: endpoint)
        return result
    }
    ...
    func login(email: String, password: String) async throws -> String {
        ...
        let shouldDeduplicate = await RequestManager.shared.shouldDeduplicateRequest(key: requestKey)
        
        if shouldDeduplicate {
            #if DEBUG
            print("⚠️ Login request deduplication: Aynı istek zaten aktif, bekleniyor...")
            #endif
            // Kısa bir süre bekle (optimize edildi - 500ms)
        ...
        let loginBody = LoginRequestBody(email: trimmedEmail, password: trimmedPassword)
        let body = try APIService.encoder.encode(loginBody)
        
        #if DEBUG
        // Request body loglanmıyor - password içeriyor, güvenlik riski
        SecureLogger.d("APIService", "Login request gönderiliyor")
        #endif
        
        do {
            // API wrapper içinde data geliyor: {"success": true, "data": {"token": "...", "user": {...}}}
            ...
            // request() fonksiyonunu kullan
            // request() zaten APIResponseWrapper'ı parse ediyor ve data kısmını döndürüyor
            let loginData: LoginDataResponse = try await request(
                endpoint: "login.php",
                method: "POST",
                body: body,
                headers: nil,
                useCache: false // Login için cache kullanma
            )
            
            // Token ve user bilgilerini güvenli şekilde kontrol et
            guard !loginData.token.isEmpty else {
                throw APIError.apiError("Giriş yanıtı geçersiz: token boş")
            }
            
            #if DEBUG
            SecureLogger.d("APIService", "Login başarılı, user ID: \(loginData.user.id)")
            #endif
            
            // Token'ı kaydet (güvenli storage)
            SecureStorage.shared.saveToken(loginData.token)
            setAuthToken(loginData.token)
            
            #if DEBUG
            // Token bilgisi loglanmıyor - güvenlik için
            SecureLogger.d("APIService", "Token kaydedildi ve doğrulandı")
            #endif
            ...
            // Network hataları için retry mekanizması (exponential backoff)
            if let urlError = error as? URLError {
                ...
                if retryableErrors.contains(urlError.code) {
                    #if DEBUG
                    print("🔄 Network hatası, retry deneniyor...")
                    #endif
                    // Retry logic burada olabilir, ama şimdilik error throw ediyoruz
                    // Task.sleep kaldırıldı - retry mekanizması zaten performRequest'te var
                }
            }
            
            if let apiError = error as? APIError {
                let errorDesc = apiError.errorDescription ?? "Bilinmeyen hata"
                #if DEBUG
                print("   API Error: \(errorDesc)")
                #endif
                
                // Daha anlaşılır hata mesajları
                if errorDesc.contains("Email veya şifre hatalı") || errorDesc.contains("email") || errorDesc.contains("şifre") {
                    throw APIError.apiError("Email veya şifre hatalı. Lütfen bilgilerinizi kontrol edin.")
                } else if errorDesc.contains("kilit") || errorDesc.contains("locked") {
                    throw APIError.apiError(errorDesc) // Kilit mesajını olduğu gibi göster
                } else if errorDesc.contains("rate limit") || errorDesc.contains("çok fazla") {
                    throw APIError.apiError(errorDesc) // Rate limit mesajını olduğu gibi göster
                } else if errorDesc.contains("Veritabanı") || errorDesc.contains("database") {
                    throw APIError.apiError("Veritabanı hatası. Lütfen daha sonra tekrar deneyin.")
                } else {
                    throw apiError
                }
            } else {
