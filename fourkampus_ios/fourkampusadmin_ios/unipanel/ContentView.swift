...
struct ContentView: View {
    ...
    private var campaignsTab: some View {
        NavigationStack(path: $campaignsNavigationPath) {
            CampaignsView(viewModel: campaignsVM, navigationPath: $campaignsNavigationPath)
        }
        .tabItem {
        ...
        .task {
            ...
            await withTaskGroup(of: Void.self) { group in
                ...
                group.addTask(priority: .userInitiated) {
                    ...
                    // Topluluklar yüklendikten sonra üyelik durumlarını da preload et
                    await communitiesVM.loadMembershipStatuses()
                }
...
extension ContentView {
    ...
    func handleDeepLink(url: URL) {
        ...
        #if DEBUG
        print("🔗 Deep link alındı: \(url.absoluteString)")
        print("   Scheme: \(scheme)")
        ...
        switch scheme {
        case "unifour", "fourkampus":
            handleCustomSchemeDeepLink(url)
        case "http", "https":
            handleWebDeepLink(url)
        default:
            #if DEBUG
            print("⚠️ Bilinmeyen scheme: \(scheme)")
            #endif
            break
        }
    }
    ...
    private func handleCustomSchemeDeepLink(_ url: URL) {
        ...
        #if DEBUG
        print("📱 Custom scheme deep link işleniyor:")
        print("   Components: \(components)")
        #endif
        
        guard !components.isEmpty else {
            #if DEBUG
            print("⚠️ Custom scheme deep link: Boş components")
            #endif
            return
        }
        ...
        switch action {
        case "community":
            guard let communityId = params.first, !communityId.isEmpty else {
                #if DEBUG
                print("⚠️ Community deep link: communityId bulunamadı")
                #endif
                return
            }
            #if DEBUG
            print("✅ Topluluk deep link tespit edildi: \(communityId)")
            #endif
        ...
        case "event":
            guard params.count >= 2 else {
                #if DEBUG
                print("⚠️ Event deep link: Yetersiz parametre (gerekli: communityId, eventId)")
                #endif
                return
            }
            let communityId = params[0]
            let eventId = params[1]
            #if DEBUG
            print("✅ Etkinlik deep link tespit edildi: community=\(communityId), event=\(eventId)")
            #endif
        ...
        default:
            #if DEBUG
            print("⚠️ Bilinmeyen action: \(action)")
            #endif
            break
        }
    }
    ...
    private func resolvePendingCommunityDeepLink() {
        guard let communityId = pendingCommunityId, !communityId.isEmpty else {
            #if DEBUG
            print("⚠️ resolvePendingCommunityDeepLink: pendingCommunityId boş")
            #endif
            return
        }
        
        #if DEBUG
        print("🔍 Topluluk aranıyor: \(communityId)")
        print("   Mevcut topluluk sayısı: \(communitiesVM.communities.count)")
        #endif
        
        if let community = communitiesVM.communities.first(where: { $0.id == communityId }) {
            #if DEBUG
            print("✅ Topluluk bulundu: \(community.name)")
            #endif
            ...
            #if DEBUG
            print("🔄 Topluluk bulunamadı, yeniden yükleniyor...")
            #endif
            ...
            #if DEBUG
            print("⚠️ Topluluk bulunamadı ve isLoading=\(communitiesVM.isLoading), didRequest=\(didRequestCommunityReloadForDeepLink)")
            #endif
        }
    }
    
    private func resolvePendingEventDeepLink(forceReload: Bool) {
        guard let pendingEventLink, !pendingEventLink.eventId.isEmpty else {
            #if DEBUG
            print("⚠️ resolvePendingEventDeepLink: pendingEventLink boş")
            #endif
            return
        }
        
        #if DEBUG
        print("🔍 Etkinlik aranıyor: eventId=\(pendingEventLink.eventId), communityId=\(pendingEventLink.communityId)")
        print("   Mevcut etkinlik sayısı: \(eventsVM.displayedEvents.count)")
        #endif
        ...
        if let event = eventsVM.findEvent(
            ...
            #if DEBUG
            print("✅ Etkinlik bulundu: \(event.title)")
            #endif
            ...
            #if DEBUG
            print("🔄 Etkinlik bulunamadı, yeniden yükleniyor...")
            #endif
            ...
            #if DEBUG
            print("⚠️ Etkinlik bulunamadı ve state=\(eventsVM.state), didRequest=\(didRequestEventReloadForDeepLink)")
            #endif
        }
    }
    ...
    private func navigateToCommunity(_ community: Community) {
        #if DEBUG
        print("🚀 Topluluğa yönlendirme: \(community.name) (id: \(community.id))")
        #endif
        ...
        Task { @MainActor in
            ...
            #if DEBUG
            print("✅ NavigationPath güncellendi - Topluluk detay sayfası açılıyor")
            #endif
        }
    }
    ...
    private func navigateToEvent(_ event: Event) {
        #if DEBUG
        print("🚀 Etkinliğe yönlendirme: \(event.title) (id: \(event.id))")
        #endif
        deepLinkEvent = event
        selectedTab = 1
        // NavigationPath'i tek bir işlemde güncelle - aynı frame içinde birden fazla güncelleme yapma
        Task { @MainActor in
            ...
            #if DEBUG
            print("✅ NavigationPath güncellendi - Etkinlik detay sayfası açılıyor")
            #endif
        }
    }
