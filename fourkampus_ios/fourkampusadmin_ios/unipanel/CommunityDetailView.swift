...
struct CommunityDetailView: View {
    ...
    private func loadMembershipStatus() {
        ...
        // Önce preloaded veriyi kontrol et - ANINDA yükleme
        if let preloadedStatus = communitiesViewModel.getMembershipStatus(for: community.id) {
            #if DEBUG
            print("✅ Preloaded üyelik durumu kullanıldı: \(community.name), isMember=\(preloadedStatus.isMember), status=\(preloadedStatus.status)")
            #endif
            membershipStatus = preloadedStatus
            return
        }
        
        #if DEBUG
        print("🔄 Preloaded veri yok, API'den üyelik durumu yükleniyor: \(community.name)")
        #endif
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    // Preloaded cache'e de ekle
                    communitiesViewModel.updateMembershipStatus(communityId: community.id, status: status)
                }
            } catch {
                await MainActor.run {
                    isLoadingMembershipStatus = false
    ...
    private func leaveCommunity() async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                // Preloaded cache'i de güncelle
                communitiesViewModel.updateMembershipStatus(communityId: community.id, status: newStatus)
