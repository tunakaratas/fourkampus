...
class CommunitiesViewModel: ObservableObject {
    ...
    // Preloaded üyelik durumları: Her topluluk için tam MembershipStatus objesi
    @Published var membershipStatuses: [String: MembershipStatus] = [:]
    ...
    func loadMembershipStatuses(forceRefresh: Bool = false) async {
        // Token kontrolü: Login olmayan kullanıcılar için üyelik durumu sorgulama
        guard SecureStorage.shared.isTokenValid() else {
            #if DEBUG
            print("ℹ️ loadMembershipStatuses: Kullanıcı login değil, sorgulama atlanıyor")
            #endif
        ...
        for (batchIndex, batch) in batches.enumerated() {
            ...
            // Her batch içinde paralel işle - MembershipStatus da döndür
            await withTaskGroup(of: (String, Bool, Bool, MembershipStatus?).self) { group in
                for communityId in batch {
                    group.addTask {
                        do {
                            ...
                            #endif
                            return (communityId, isMember, true, status)
                        } catch {
                            ...
                            return (communityId, false, false, nil)
                        }
                    }
                }
                ...
                for await (communityId, isMember, success, status) in group {
    ...
    /// Preloaded üyelik durumunu getir
    /// CommunityDetailView bu fonksiyonu çağırarak anında üyelik durumunu alabilir
    func getMembershipStatus(for communityId: String) -> MembershipStatus? {
        membershipStatuses[communityId]
    }
    
    /// Üyelik durumunu güncelle (topluluktan ayrılma/katılma sonrası)
    func updateMembershipStatus(communityId: String, status: MembershipStatus) {
        membershipStatuses[communityId] = status
        
        // memberCommunityIds'i de güncelle
        let isMember = status.isMember || status.status == "member" || status.status == "approved"
        if isMember {
            memberCommunityIds.insert(communityId)
        } else {
            memberCommunityIds.remove(communityId)
        }
    }
...
class CampaignsViewModel: ObservableObject {
    ...
    func loadCampaigns(universityId: String? = nil, forceRefresh: Bool = false) async {
        // forceRefresh true ise isLoading kontrolünü atla
        ...
        #if DEBUG
        print("🔄 CampaignsViewModel: Kampanyalar yükleniyor... (forceRefresh: \(forceRefresh))")
        #endif
        ...
        do {
            ...
            // Kullanıcı dostu hata mesajı - sadece ilk yüklemede göster
            if !hasInitiallyLoaded {
                errorMessage = ErrorHandler.userFriendlyMessage(from: error)
            }
            
            // ÖNEMLİ: Hata durumunda mevcut kampanyaları SİLME!
            // Sadece ilk yükleme başarısız olursa boş bırak
            if !hasInitiallyLoaded {
                campaigns = []
            }
            // Aksi halde mevcut kampanyaları koru
    ...
    func refreshCampaigns(universityId: String? = nil) async {
        ...
        // forceRefresh: true ile yenileme yap - isLoading kontrolünü atla
        await loadCampaigns(universityId: nil, forceRefresh: true)
    }
...
class MarketViewModel: ObservableObject {
    ...
    func loadProducts(universityId: String? = nil, isRefresh: Bool = false) async {
        // isRefresh true ise isLoading kontrolünü atla (zorla yenile)
