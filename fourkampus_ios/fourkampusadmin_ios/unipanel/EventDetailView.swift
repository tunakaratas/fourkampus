...
struct EventDetailView: View {
    ...
    // Görüntülenecek etkinlik (yenilenmiş veri varsa onu kullan)
    ...
    var body: some View {
        ...
        .onAppear {
            // Detayları her zaman yenile (eksik veri sorununu çözmek için)
            loadEventDetails()
    ...
    private func loadMembershipStatus() {
        ...
        if let preloadedStatus = communitiesViewModel.getMembershipStatus(for: currentEvent.communityId) {
            #if DEBUG
            print("✅ EventDetailView: Preloaded üyelik durumu kullanıldı: \(preloadedStatus.status)")
            #endif
        ...
        Task {
            do {
                #if DEBUG
                print("🔄 EventDetailView: Preloaded veri yok, API'den üyelik durumu yükleniyor: \(currentEvent.communityId)")
                #endif
    ...
    private func loadEventDetails() {
        Task {
            do {
                // Etkinlik detaylarını taze olarak çek
                let details = try await APIService.shared.getEvent(id: event.id)
                await MainActor.run {
                    self.refreshedEvent = details
                    
                    // Detaylar geldikten sonra media kontrolleri vb. otomatik güncellenir
                    // Eğer üyelik durumu da gerekiyorsa tekrar kontrol edilebilir
                    if details.isMember != nil {
                        // API'den gelen veride üyelik bilgisi varsa kullanabiliriz
                        // Ama şimdilik mevcut mantığı koruyoruz
                    }
                }
            } catch {
                print("⚠️ Event detail refresh failed: \(error.localizedDescription)")
            }
        }
    }
}
